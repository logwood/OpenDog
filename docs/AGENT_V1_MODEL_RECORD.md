# Agent experiment / MegaDescriptor model record (historical revision V1)

> `V1` in this file is the historical Agent experiment revision. It is not a
> UnifiedPetReID model generation or a production deployment version.

This Agent experiment keeps the existing BIFOR 512-D descriptor as the primary identity
...
Decision at the time of this experiment: keep BIFOR as that experiment's
primary identity score. Retain MegaDescriptor features and the Agent path as
an experimental expert for guarded rescue, reranking, or future calibrated
gating; do not use the roughly half-weight score fusion as an unconditional
default. This historical decision does not describe the current production
pointer, which is recorded separately in `models/registry.json`.
