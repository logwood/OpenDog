...
From `src/Pet-ReID-IMAG`:

```powershell
& $Python tools\multimodal_inference.py `
  image_a.jpg image_b.jpg `
  --config-file configs\multimodal_inference.yaml `
  --output-dir ..\..\artifacts\evaluations\multimodal_run
```
...
```powershell
& $Python tools\prepare_dogfacenet_alignment.py `
  ..\..\data\raw\DogFaceNet_alignment `
  --archive ..\..\archive\downloads\DogFaceNet_alignment.zip `
  --min-eye-distance 128 `
  --output-dir ..\..\artifacts\evaluations\dogfacenet_alignment_geometry
```
...
```powershell
& $Python tools\train_multimodal_dogfacenet.py `
  ..\..\artifacts\evaluations\dogfacenet_alignment_geometry\manifest.json `
  --steps 1000 --identities-per-batch 4 --images-per-identity 2 `
  --output-dir ..\..\artifacts\runs\multimodal-dogfacenet\manual-1000-step
```
...
& $Python tools/evaluate_multimodal_dogfacenet.py `
  ..\..\artifacts\evaluations\dogfacenet_alignment_geometry\manifest.json `
  ..\..\artifacts\runs\multimodal-dogfacenet\manual-1000-step\model_final.pth `
  --output-dir ..\..\artifacts\evaluations\multimodal_dogfacenet_eval
```
...
& $Python tools/evaluate_multimodal_dogfacenet.py `
  ...
  --visualization ..\..\artifacts\evaluations\frozen_pretrained_pairs.png
```
...
& $Python tools/multimodal_inference.py image_a.jpg image_b.jpg `
  --identity-weights ..\..\artifacts\runs\multimodal-dogfacenet\manual-1000-step\model_final.pth
```
