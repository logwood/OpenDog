...
## Role-anchored implementation status

The planned replacement is now implemented in a new, isolated
`RoleAnchoredLatentWorkspace`. Existing latent configs retain their historical
defaults and output directories.

The role-anchored data flow is:
...
.\scripts\train_role_anchored_latent.ps1 -PreflightOnly

# Four optimizer steps: data, AMP, forward/backward, validation and checkpoint.
.\scripts\train_role_anchored_latent.ps1 -Smoke
...
.\scripts\train_role_anchored_latent.ps1 -Microfit

# Only after all three gates pass:
.\scripts\train_role_anchored_latent.ps1
```
...
& $Python tools\preflight_role_anchored_latent.py `
  --config-file configs\modern_latent_role_anchored_s101_224.yaml `
  --weights ..\..\artifacts\runs\legacy\modern_latent_role_anchored_s101_224_d192\model_0002.pth `
  --device cuda
```

The PowerShell launchers now write new experiments to the standard
`artifacts/runs/<workstream>/<run-id>` layout and create a run manifest.
Use `-LegacyLayout` only when intentionally continuing a retained pre-cleanup
checkpoint.
