...
资产准备脚本会把 ZIP 内含 `*` 的测试文件名安全映射为 Windows 可存储的 `_` 名称，并生成 `data/processed/pet-reid-imag/test/filename_map.csv`。模型内部始终把它映射回比赛原名，因此 pair CSV、四分支特征和最终提交不会错位。
...
它使用 8 个训练身份和 64 个验证对，真实执行 ResNeSt-101 混合精度训练、CrossEntropy/Triplet/Circle 三项损失、ROC-AUC、best/final checkpoint。Ada/Ampere 等支持 BF16 的 GPU 自动使用 BF16，其他 CUDA GPU 回退 FP16+GradScaler。

主入口默认把每次新运行写到
`artifacts/runs/<workstream>/<run-id>/`，其中包含 manifest、resolved config、
metrics、stdout、TensorBoard、checkpoint 和报告。需要恢复时传入第一次运行打印的
`RunId`：

```powershell
.\scripts\run_modern_pipeline.ps1 -Mode Train -Branch s101_224 -RunId validation-01
.\scripts\run_modern_pipeline.ps1 -Mode Resume -Branch s101_224 -RunId validation-01
```

若确实要继续整理前的 `artifacts/runs/legacy` checkpoint，显式添加
`-LegacyLayout`；默认不会把新实验写回 legacy 目录。
...
这些配置只用 5,400 个训练身份，并每个 epoch 在 600 个未见身份构造的 2,000 个验证对上输出 `ROC_AUC`。输出分别位于对应的
`artifacts/runs/reproduction-validation-<branch>/<run-id>/`。
...
对应 `modern_final_*.yaml` 配置。全量训练不再运行留出验证，checkpoint 写到
`artifacts/runs/reproduction-final-<branch>/<run-id>/checkpoints/`。四个分支应逐个执行；8GB 单卡不要并行启动。
...
两份官方 ImageNet 初始化权重已位于 `models/pretrained/`，SHA-256 分别以 `22405ba7` 和 `75117900` 开头，并已实际构建 ResNeSt-101/200 模型验证加载。
...
```text
artifacts/runs/legacy/fusion_submit/submit_modern.csv
```
...
输出位于 `artifacts/runs/legacy/retrained_fusion/submit.csv`。
...
输出为 `artifacts/reports/phase_b_pair_examples.png`，图内也明确标注了“无隐藏标签，不能据此宣称预测正确”。
