...
> 当前执行状态（2026-09-01）：目录迁移、唯一 Git 所有权、旧路径兼容和功能验收已经完成；checkpoint、压缩包及其他破坏性清理仍未执行。本文前半部分保留“整理前基线”和逐项计划，当前真实状态以第 13 节和 [`SESSION_STATE_CN.md`](SESSION_STATE_CN.md) 为准。
...
以下是 2026-08-28 整理前盘点得到的主要情况，不代表当前目录仍保持这些旧路径；实验产物在后续阶段可能继续增长。
...
整理后根目录的 `DogFaceNet_alignment/` 仍可看到，但它是指向
`data/raw/DogFaceNet_alignment/` 的 junction，仅为旧脚本兼容，不是重复数据。
...
## 13. 本次执行记录（更新至 2026-09-01）

本轮已完成源码归属、目录迁移和运行路径硬化；随后又完成 UnifiedPetReID V4 高分辨率候选的独立发布与验证。未执行任何永久删除或 checkpoint 隔离，因此原始数据、模型、图库和压缩包仍可恢复。

| 项目 | 结果 | 证据 |
|---|---|---|
| 外层 Git 唯一所有权 | 已完成 | 工作区递归只剩根目录 `.git`；内层仓库均有 bundle 和 quarantine 备份 |
| 源码/数据/模型/产物分区 | 已完成 | `src/`、`data/`、`models/`、`artifacts/`、`archive/` |
| 旧数据路径兼容 | 已完成 | 根目录 `DogFaceNet_alignment/` 是指向 `data/raw/DogFaceNet_alignment/` 的 junction；实际数据只有一份 |
...
| UnifiedPetReID V4 | 当前研发主线（已验证候选） | `models/selected/unified_pet_reid_v4_v1/`；V3 仍是生产基线和默认部署，V4 使用独立图库 |

注意：第 5、6、8 阶段中涉及隔离或永久删除的勾选项保持未选中是有意的；需要人工确认后再执行。旧实验无法可靠恢复的 command、seed、Git commit 或指标在集中清单中保持空值，不伪造 manifest。根目录 junction 是兼容层，不应删除。连续运行 `scripts/generate_workspace_metadata.py` 时，legacy 清单内容与 mtime 均保持幂等，`models/registry.json` 也不会仅因时间戳产生内容差异。若聊天任务发生压缩或中断，请先查看 [`SESSION_STATE_CN.md`](SESSION_STATE_CN.md)，不要把会话缺段当成工作区文件丢失。
