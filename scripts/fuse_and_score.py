...
def fuse(root: Path, model_dirs=MODELS):
    qs, gs = [], []
    for m in model_dirs:
        q = np.load(root / "logs" / m / "query_f.npy")
        g = np.load(root / "logs" / m / "gallery_f.npy")
...
def score_pairs(root: Path, pairs_csv: Path, query_names: Path, gallery_names: Path, output: Path, model_dirs=MODELS):
    qf, gf = fuse(root, model_dirs)
...
def self_test():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for j,m in enumerate(MODELS):
            ...
            p = root / "logs" / m; p.mkdir(parents=True)
        ...
        out = root/"submit.csv"
        rep = score_pairs(root, root/"pairs.csv", root/"query_filename.txt", root/"gallery_filename.txt", out)
        with out.open() as f:
...
def main():
    ...
    ap.add_argument("--pairs", default="data/test/test_data.csv")
    ap.add_argument("--query-names", default="logs/s101_224/query_filename.txt")
    ap.add_argument("--gallery-names", default="logs/s101_224/gallery_filename.txt")
    ap.add_argument("--output", default="logs/fusion_submit/submit_fixed.csv")
    ap.add_argument(
        "--model-dirs",
        nargs=4,
        default=list(MODELS),
    ...
    rep=score_pairs(
        root,
        root/args.pairs,
        root/args.query_names,
        root/args.gallery_names,
        root/args.output,
        args.model_dirs,
    )
