...
class LiveStackSmoke:
    def __init__(self, args: argparse.Namespace, output: Path) -> None:
        ...
        self.report: dict[str, Any] = {
            ...
            "expect_single_graph": args.expect_single_graph,
    ...
    def run(self) -> None:
        ...
        self.record_check(
            "gateway reports expected inference backend",
            backend.get("backend") == args.expected_backend,
            backend.get("backend"),
        )
        self.record_check(
            "gateway reports expected deployment profile",
            backend.get("deployment_profile") == args.expected_profile,
            backend.get("deployment_profile"),
        )
        self.record_check(
            "gateway reports expected model capability",
            backend.get("capability") == args.expected_capability,
            backend.get("capability"),
        )
        if args.expected_fusion:
            self.record_check(
        ...
        if args.expect_single_graph:
            self.record_check(
                "gateway reports the locked single-graph architecture",
                backend.get("single_graph") is True
                and backend.get("external_models") == []
                and backend.get("raw_spatial_input") is True
                and backend.get("model_sha256") == args.expected_model_sha256,
                {
                    "single_graph": backend.get("single_graph"),
                    "external_models": backend.get("external_models"),
                    "raw_spatial_input": backend.get("raw_spatial_input"),
        ...
        if args.expected_capability in {
            "semantic-body-fusion",
            "multi-expert-evidence",
        }:
            self.record_check(
        ...
        self._assert_identification(
            identification,
            inputs["pet_a_id"],
            require_dual=not args.allow_single_branch and not args.expect_single_graph,
        )
    ...
    def _assert_identification(
        ...
        if self.args.expect_single_graph:
            unified = (descriptor.get("runtime_diagnostics") or {}).get("unified") or {}
            self.record_check(
                "query used one UnifiedPetReID graph without external models",
                unified.get("single_graph") is True
                and unified.get("external_models") == []
                and unified.get("raw_spatial_input") is True
                and descriptor.get("branch_available") is None
                and descriptor.get("fusion_weights") is None,
                unified,
            )
        if self.args.expected_fusion == "semantic_residual+bifor_lowrank":
        ...
        if self.args.expected_agent:
            ...
            self.record_check(
                ...
                result.get("decision") == "agent_evidence"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=(
            "onnxruntime-unified",
            "onnxruntime-unified-highres",
            "onnxruntime",
            "onnxruntime-bifor",
        ),
        default="onnxruntime-unified",
    )
    parser.add_argument(
        "--expected-model-sha256",
        default="",
        help="exact model fingerprint required by --expect-single-graph",
    )
    parser.add_argument("--expected-profile", default="production")
    parser.add_argument("--expected-capability", default="unified-embedding")
    parser.add_argument(
        "--expected-fusion",
        default=None,
    )
    parser.add_argument("--expect-single-graph", action="store_true")
