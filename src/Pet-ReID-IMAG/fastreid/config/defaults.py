...
# A negative value preserves the historical generated-seed behavior. Experiment
# configs can opt into an exact, rank-offset seed for reproducible training.
_C.SEED = -1
...
# Persistent latent workspace.  This branch is only constructed by the
# LatentWorkspaceBaseline meta architecture; the released Baseline path does
# not consume these options.
_C.MODEL.LATENT_WORKSPACE = CN()
_C.MODEL.LATENT_WORKSPACE.DIM = 192
_C.MODEL.LATENT_WORKSPACE.NUM_SLOTS = 8
_C.MODEL.LATENT_WORKSPACE.NUM_HEADS = 8
_C.MODEL.LATENT_WORKSPACE.MLP_RATIO = 4.0
_C.MODEL.LATENT_WORKSPACE.GATE_MIN = 0.05
_C.MODEL.LATENT_WORKSPACE.GATE_INIT = 0.10
_C.MODEL.LATENT_WORKSPACE.ATTN_DROPOUT = 0.0
_C.MODEL.LATENT_WORKSPACE.HEALTH_PERIOD = 200
_C.MODEL.LATENT_WORKSPACE.READ_MODE = "mha"
_C.MODEL.LATENT_WORKSPACE.MESH_ITERS = 4
_C.MODEL.LATENT_WORKSPACE.SINKHORN_ITERS = 5
_C.MODEL.LATENT_WORKSPACE.MESH_LR = 1.0
_C.MODEL.LATENT_WORKSPACE.MESH_NOISE_STD = 0.001
_C.MODEL.LATENT_WORKSPACE.MESH_TEMPERATURE = 1.0
_C.MODEL.LATENT_WORKSPACE.MIX_GATE_INIT = 0.05
_C.MODEL.LATENT_WORKSPACE.MIX_GATE_TRAINABLE = True
_C.MODEL.LATENT_WORKSPACE.ROLE_ANCHOR_ENABLED = False
_C.MODEL.LATENT_WORKSPACE.ROLE_ANCHOR_SCALE = 0.25
_C.MODEL.LATENT_WORKSPACE.POSITION_ENCODING_ENABLED = False
_C.MODEL.LATENT_WORKSPACE.POSITION_ENCODING_SCALE = 1.0
_C.MODEL.LATENT_WORKSPACE.UPDATE_MODE = "replace"
_C.MODEL.LATENT_WORKSPACE.UPDATE_ALPHAS = [1.0, 1.0, 1.0, 1.0]
_C.MODEL.LATENT_WORKSPACE.MIX_ENABLED = True
_C.MODEL.LATENT_WORKSPACE.FFN_LAYER_SCALE = 1.0
_C.MODEL.LATENT_WORKSPACE.MIX_ROLE_CONDITIONED = False
_C.MODEL.LATENT_WORKSPACE.MIX_CENTERED = False
_C.MODEL.LATENT_WORKSPACE.WRITE_STAGES = ["c3", "c4", "c5"]
_C.MODEL.LATENT_WORKSPACE.SPATIAL_WRITE_GATE_ENABLED = False
_C.MODEL.LATENT_WORKSPACE.SLOT_SET_HEAD_ENABLED = False
_C.MODEL.LATENT_WORKSPACE.SLOT_SET_DIM_PER_SLOT = 64
_C.MODEL.LATENT_WORKSPACE.SLOT_SET_LOSS_WEIGHT = 0.20
_C.MODEL.LATENT_WORKSPACE.SLOT_FEATURE_FUSION_SCALE = 0.20
_C.MODEL.LATENT_WORKSPACE.SLOT_INFERENCE_FUSION_WEIGHT = 0.20
_C.MODEL.LATENT_WORKSPACE.IDENTITY_QUERY_HEAD_ENABLED = False
_C.MODEL.LATENT_WORKSPACE.IDENTITY_QUERY_NUM = 4
_C.MODEL.LATENT_WORKSPACE.IDENTITY_QUERY_DIM = 128
_C.MODEL.LATENT_WORKSPACE.IDENTITY_QUERY_NUM_HEADS = 4
_C.MODEL.LATENT_WORKSPACE.IDENTITY_QUERY_FFN_SCALE = 0.10
_C.MODEL.LATENT_WORKSPACE.IDENTITY_QUERY_LOSS_WEIGHT = 0.20
_C.MODEL.LATENT_WORKSPACE.IDENTITY_FUSION_LOSS_WEIGHT = 0.10
_C.MODEL.LATENT_WORKSPACE.IDENTITY_FUSION_WEIGHT_MIN = 0.0
_C.MODEL.LATENT_WORKSPACE.IDENTITY_FUSION_WEIGHT_MAX = 0.5
_C.MODEL.LATENT_WORKSPACE.IDENTITY_FUSION_WEIGHT_INIT = 0.05
_C.MODEL.LATENT_WORKSPACE.EARLY_ABORT_ENABLED = False
_C.MODEL.LATENT_WORKSPACE.EARLY_ABORT_WARMUP_ITERS = 100
_C.MODEL.LATENT_WORKSPACE.EARLY_ABORT_PATIENCE = 2
_C.MODEL.LATENT_WORKSPACE.EARLY_ABORT_SLOT_COSINE_MAX = 0.995
_C.MODEL.LATENT_WORKSPACE.EARLY_ABORT_MIN_EFFECTIVE_RANK = 2.0
_C.MODEL.LATENT_WORKSPACE.EARLY_ABORT_QUERY_COSINE_MAX = 0.995
_C.MODEL.LATENT_WORKSPACE.EARLY_ABORT_MIN_QUERY_RANK = 1.5
