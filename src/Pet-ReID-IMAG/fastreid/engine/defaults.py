...
def default_setup(cfg, args):
    ...
    # Keep historical generated seeds unless an experiment explicitly requests
    # reproducibility. Distributed workers receive deterministic rank offsets.
    configured_seed = None if cfg.SEED < 0 else int(cfg.SEED) + rank
    seed_all_rng(configured_seed)
...
class DefaultTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        checkpointables = {"optimizer": optimizer, **self.scheduler}
        grad_scaler = getattr(self._trainer, "grad_scaler", None)
        if grad_scaler is not None:
            checkpointables["grad_scaler"] = grad_scaler
    ...
    def train(self):
        ...
        if comm.is_main_process():
            if not self.cfg.DATASETS.TESTS:
                # A final train-on-all-data fit intentionally has no disjoint
                # evaluation set. Checkpoints are still written normally.
                return {}
            assert hasattr(
    ...
    def build_optimizer(cls, cfg, model):
        ...
        """
        # The legacy contiguous-gradient buffer relies on every parameter
        # keeping a preallocated ``.grad`` view.  Modern PyTorch optimizers and
        # GradScaler use ``set_to_none=True`` semantics, which invalidates that
        # assumption and leaves AMP with no gradients to inspect.  Native
        # parameter groups are fully supported and are the stable PyTorch 2.x
        # path.
