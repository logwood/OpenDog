...
def inference_context(model):
    ...
    and every submodule is restored to its previous mode afterwards.

    A single top-level ``model.training`` flag is insufficient when training
    intentionally mixes modes, e.g. a frozen backbone in eval mode with a
    train-mode head/workspace. ``model.train(True)`` would recursively reopen
    the frozen backbone after validation.
    Args:
    ...
    try:
        ...
        # ``modules()`` is parent-first. Restoring in that order lets a saved
        # child-specific mode override the recursive mode set by its parent.
