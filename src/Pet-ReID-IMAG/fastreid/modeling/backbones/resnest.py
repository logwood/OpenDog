...
class ResNeSt(nn.Module):
    ...
    def forward_stem(self, x):
        """Run the convolutional stem without changing the released forward path."""
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
    ...
    def forward_stage(self, x, stage):
        """Run one numbered ResNeSt stage (1 through 4)."""
        if stage == 1:
        ...
        raise ValueError(f"ResNeSt stage must be in [1, 4], got {stage}")
