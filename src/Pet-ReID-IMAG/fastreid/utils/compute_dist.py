...
def compute_jaccard_distance(features, k1=20, k2=6, search_option=0, fp16=False):
    # FAISS is only needed for optional Jaccard re-ranking. Importing it at
    # module load time made the default cosine training/inference path fail on
    # machines where faiss-gpu is unavailable (notably Windows and newer CUDA
    # environments).
    try:
        import faiss
        ...
        raise ImportError(
            "FAISS is required only when TEST.RERANK.ENABLED is true. "
            "Install a CUDA-compatible faiss-gpu build or disable re-ranking."
        ) from exc
