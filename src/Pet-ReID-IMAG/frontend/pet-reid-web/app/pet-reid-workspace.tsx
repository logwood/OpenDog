    ...
    <section className="result-panel panel" aria-live="polite">
      ...
      <div className="result-main">
        ...
        <div className="score-grid">
          ...
          <div>
            <span>特征分支</span>
            <strong className={fusion === "joint" ? "good-text" : fusion === "fallback" ? "warn-text" : ""}>
              {fusion === "joint" ? "鼻子 + 脸" : fusion === "fallback" ? "单路回退" : "未知"}
            </strong>
            <small>{fusion === "joint" ? "双分支" : fusion === "fallback" ? "单分支" : "无分支信息"}</small>
          </div>
        </div>
    ...
    <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && close()}>
      <section className="dialog-card" role="dialog" aria-modal="true" aria-labelledby="enroll-title">
        ...
        <form onSubmit={submit}>
          ...
          <button className="mini-upload" type="button" onClick={() => inputRef.current?.click()}><span>+</span><strong>{files.length ? "继续添加参考图" : "选择 1–8 张参考图"}</strong><small>JPG / PNG / WebP / BMP</small></button>
      ...
      <aside className="pet-drawer" role="dialog" aria-modal="true" aria-labelledby="pet-detail-title">
        ...
        {pet && !loading ? (
          <>
            ...
            <div className="drawer-toolbar"><button type="button" disabled={busyId === "__add__"} onClick={() => addImageRef.current?.click()}>{busyId === "__add__" ? "处理中…" : "+ 补充参考图"}</button><input ref={addImageRef} className="sr-only" type="file" multiple accept="image/jpeg,image/png,image/webp,image/bmp" onChange={(event) => void addImages(Array.from(event.target.files ?? []))} /></div>
    ...
    <main className="app-shell">
      ...
      <div className="workspace">
        ...
        <section className="content">
          <div className="intro"><div><h1>宠物图片比对</h1></div><p className="intro-copy">选择一张图片，与临时图库中的身份进行比较。</p></div>
          ...
          <div className="dashboard-grid" id="identify">
            <section className="panel identify-panel">
              <div className="panel-heading"><div><h2>选择图片</h2></div><span className="privacy-note">本机处理</span></div>
              <label className={"upload-zone " + (queryFile ? "has-file " : "") + (dragging ? "dragging" : "")} onDragEnter={(event: DragEvent<HTMLLabelElement>) => { event.preventDefault(); setDragging(true); }} onDragOver={(event: DragEvent<HTMLLabelElement>) => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={(event: DragEvent<HTMLLabelElement>) => { event.preventDefault(); setDragging(false); chooseQuery(event.dataTransfer.files[0]); }}>
                ...
                {queryPreview ? <div className="query-preview"><img src={queryPreview} alt="待比对宠物照片预览" /><span>点击可更换照片</span></div> : <><span className="upload-orbit" aria-hidden="true"><span>+</span></span><strong>{dragging ? "松开即可选择" : "拖入照片，或点击选择"}</strong><small>支持 JPG、PNG、WebP、BMP · 单张不超过 15 MB</small></>}
              </label>
              ...
              <div className="upload-footer"><p><span />图片中只保留一只宠物</p><button type="button" disabled={!queryFile || identifying} onClick={() => void identify()}>{identifying ? <><i className="spinner" />比对中…</> : queryFile ? "开始比对" : "请选择图片"}</button></div>
            </section>

            <aside className="summary-stack" id="system">
              <section className="panel model-card"><span className="card-index">模型</span><h2>{modelLabel(health)}</h2><p>{backendValue(health, "embedding_dim") === "—" ? "512" : backendValue(health, "embedding_dim")} 维特征</p><dl><div><dt>后端</dt><dd>{providerLabel(health)}</dd></div><div><dt>指纹</dt><dd title={health?.model_fingerprint}>{fingerprint}</dd></div></dl></section>
          ...
          <AdminSection onGalleryChanged={onGalleryChanged} />
        </section>
      </div>
