    ...
    return (
      <section className="tool-section admin-locked" id="admin">
        ...
        <form onSubmit={(event) => { event.preventDefault(); void unlock(keyInput); }}>
          <label><span>管理员密钥</span><input type="password" value={keyInput} onChange={(event) => setKeyInput(event.target.value)} placeholder="artifacts/workspace_logs/quick_start/admin-key.txt" autoComplete="off" /></label>
