...
class AgentEvidenceRequest:
    ...
    def from_identification(
        ...
        *,
        ...
        """Convert production, candidate, or legacy output without leaking vectors."""
...
class InProcessAgent:
    """Adapt a host-supplied callback to the validated Agent contract.

    This is the bridge for a temporary session-level provider: an orchestrator
    (including a human or an assistant session) receives the redacted request
    mapping and returns only the decision JSON. It does not expose the current
    ChatGPT/Codex session as a local HTTP service and must not be mistaken for
    a learned or remotely hosted provider.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if not callable(callback):
            raise AgentConfigurationError("in-process callback must be callable")
        provider = str(provider).strip()
        if not provider:
            raise AgentConfigurationError("in-process provider name cannot be blank")
        self.callback = callback
        self.provider = provider

    def decide(self, request: AgentEvidenceRequest) -> AgentDecision:
        # Pass only the serialized/redacted boundary to the callback. This
        # keeps the temporary provider subject to the same data contract as a
        # remote provider.
        payload = self.callback(request.to_dict())
        if not isinstance(payload, Mapping):
            raise AgentProtocolError("in-process provider must return a JSON object")
