...
def add_retri_config(cfg):
    ...
    _C.MULTIMODAL.NOSE_CONFIG = "artifacts/runs/legacy/s101_224/config.yaml"
    _C.MULTIMODAL.NOSE_WEIGHTS = "artifacts/runs/legacy/s101_224/model_final.pth"
    _C.MULTIMODAL.IDENTITY_WEIGHTS = ""
    _C.MULTIMODAL.ARCFACE_WEIGHTS = "models/pretrained/dog.pt"
    _C.MULTIMODAL.ANYFACE_ROOT = "third_party/AnyFace"
    _C.MULTIMODAL.ANYFACE_WEIGHTS = (
        "models/pretrained/anyface/yolov5l6_best.pt"
    )
    _C.MULTIMODAL.SAM2_CHECKPOINT = (
        "models/pretrained/sam2/sam2.1_hiera_tiny.pt"
    )
    ...
    _C.MULTIMODAL.CACHE_DIR = "artifacts/runs/legacy/multimodal_cache"
