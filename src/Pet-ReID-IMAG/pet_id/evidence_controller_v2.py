...
V2 treats the raw BIFOR score and margin as strong ordered evidence.  It learns
...
_OUTPUT_INDEX = {name: index for index, name in enumerate(OUTPUT_NAMES_V2)}
_CANDIDATE_INDEX = {
...
class MonotonicResidualController(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.residual = nn.Linear(len(SCALAR_FEATURE_NAMES), len(OUTPUT_NAMES_V2))
        ...
        mask = torch.ones(len(OUTPUT_NAMES_V2), len(SCALAR_FEATURE_NAMES))
...
def choose_action_v2(
    probabilities: Mapping[str, float],
    *,
    expert_available: bool,
    costs: Mapping[str, float] | None = None,
) -> dict[str, object]:
    action_costs = {**DEFAULT_COSTS_V2, **dict(costs or {})}
