...
def configure_sam2_import_path(repository_root=None) -> Path:
    """Make the vendored official SAM 2 package importable after relocation."""

    root = (
        Path(repository_root).expanduser().resolve()
        if repository_root is not None
        else (Path(__file__).resolve().parents[1] / "third_party" / "sam2").resolve()
    )
    package = root / "sam2"
    if not package.is_dir():
        raise FileNotFoundError(
            f"SAM 2 source package was not found under {root}; "
            "restore src/Pet-ReID-IMAG/third_party/sam2"
        )
...
def _sam2_hydra_config_name(config) -> str:
    """Convert a workspace/config spelling into SAM 2's Hydra config name.

    ``hydra.compose`` is initialized against the vendored ``sam2`` config
    module and therefore rejects a filesystem absolute path.  Runtime config
    normalization intentionally stores canonical absolute paths, so convert
    those paths back to the package-relative ``configs/...`` name at the
    boundary where SAM 2 is constructed.
    """

    from .workspace_paths import SAM2_CONFIG_ROOT, resolve_sam2_config_path

    resolved = resolve_sam2_config_path(config)
    config_root = SAM2_CONFIG_ROOT.resolve()
    try:
        relative = resolved.resolve().relative_to(config_root)
    except ValueError as exc:
        raise ValueError(
            "SAM2_CONFIG must resolve inside the vendored SAM 2 config "
            f"directory ({config_root}), got {resolved}"
        ) from exc
    if not relative.name:
        raise ValueError(f"SAM2_CONFIG points to a directory: {resolved}")
    return Path("configs", relative).as_posix()
...
class SAM2NoseSegmenter:
    ...
    def __init__(
        ...
        *,
        ...
        configure_sam2_import_path()
        from sam2.build_sam import build_sam2
        from sam2.sam2_image_predictor import SAM2ImagePredictor

        self.device = torch.device(device or ("cuda" if torch.cuda.is_available() else "cpu"))
        config_name = _sam2_hydra_config_name(config)
        model = build_sam2(
            config_name,
            str(checkpoint),
            device=self.device,
