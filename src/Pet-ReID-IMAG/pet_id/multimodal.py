...
from .workspace_paths import normalize_runtime_config
from .release_compatibility import normalize_fusion_mode


QUALITY_DIM = 6
LEGACY_CONCAT_FUSION = "legacy_concat"
SHARED_PROJECTION_FUSION = "shared_projection"
SEMANTIC_RESIDUAL_FUSION = "semantic_residual"
SHARED_FUSION_MODES = {SHARED_PROJECTION_FUSION, SEMANTIC_RESIDUAL_FUSION}
FUSION_MODES = {LEGACY_CONCAT_FUSION, *SHARED_FUSION_MODES}


@dataclass(frozen=True)
class PetDescriptor:
    """Identity descriptors and optional branch metadata for one animal.

    Legacy detector backends populate the nose/face branch fields. A unified
    graph has one learned embedding and deliberately leaves those fields
    unset instead of manufacturing a pair of synthetic branches. Keeping the
    container allows old gallery consumers to deserialize records while the
    metadata contract accurately describes the model that produced them.
    """

    fused_feature: torch.Tensor
    nose_feature: torch.Tensor | None
    face_feature: torch.Tensor | None
    fusion_weights: tuple[float, float] | None
    branch_quality: tuple[float, float] | None
    branch_available: tuple[bool, bool] | None
    ...
    def __post_init__(self):
        for name in ("fused_feature", "nose_feature", "face_feature"):
            ...
            if value is None:
                if name == "fused_feature":
                    raise ValueError("fused_feature is required")
                continue
            if value.ndim != 1 or not torch.isfinite(value).all():
                raise ValueError(f"{name} must be one finite descriptor vector")
        if self.fusion_weights is not None and len(self.fusion_weights) != 2:
            raise ValueError("fusion_weights must contain two values when present")
        if self.branch_quality is not None and len(self.branch_quality) != 2:
            raise ValueError("branch_quality must contain two values when present")
        if self.branch_available is not None and len(self.branch_available) != 2:
            raise ValueError("branch_available must contain two values when present")
    ...
    def metadata_dict(self) -> dict:
        ...
        return {
            "fusion_weights": (
                list(self.fusion_weights) if self.fusion_weights is not None else None
            ),
            "branch_quality": (
                list(self.branch_quality) if self.branch_quality is not None else None
            ),
            "branch_available": (
                list(self.branch_available)
                if self.branch_available is not None
                else None
            ),
...
def compare_descriptors(left: PetDescriptor, right: PetDescriptor) -> PairSimilarity:
    ...
    nose = None
    if (
        left.branch_available is not None
        and right.branch_available is not None
        and left.branch_available[0]
        and right.branch_available[0]
        and left.nose_feature is not None
        and right.nose_feature is not None
    ):
    ...
    if (
        left.branch_available is not None
        and right.branch_available is not None
        and left.branch_available[1]
        and right.branch_available[1]
        and left.face_feature is not None
        and right.face_feature is not None
    ):
...
class LocalEndToEndPetIDModel(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if (
            self.fusion_mode == SEMANTIC_RESIDUAL_FUSION
            and self.joint_dim != face_encoder.feature_dim
        ):
            raise ValueError(
                "semantic_residual requires joint_dim to equal the raw "
    ...
    def forward(
        ...
        *,
        ...
        if targets is not None:
            ...
            if (
                self.fusion_mode == SHARED_PROJECTION_FUSION
...
class DescriptorCache:
    ...
    def save(self, image_path, descriptors: Sequence[PetDescriptor]) -> Path:
        ...
        for index, descriptor in enumerate(descriptors):
            ...
            # The historical cache schema has nose/face slots.  A unified
            # descriptor has no such branches, so retain the canonical vector
            # only as a storage compatibility value and mark the branches
            # unavailable in metadata.
            nose = (
                descriptor.nose_feature
                if descriptor.nose_feature is not None
                else descriptor.fused_feature
            )
            face = (
                descriptor.face_feature
                if descriptor.face_feature is not None
                else descriptor.fused_feature
            )
            arrays[f"nose_{index}"] = nose.numpy()
            arrays[f"face_{index}"] = face.numpy()
    ...
    def load(self, image_path) -> list[PetDescriptor] | None:
        ...
        for index, item in enumerate(metadata["descriptors"]):
            ...
            unified = bool(
                ...
                and isinstance(item["runtime_diagnostics"].get("unified"), dict)
                and item["runtime_diagnostics"]["unified"].get("single_graph") is True
            )
            descriptors.append(
                PetDescriptor(
                    ...
                    nose_feature=(
                        None
                        if unified
                        else torch.from_numpy(features[f"nose_{index}"].copy())
                    ),
                    face_feature=(
                        None
                        if unified
                        else torch.from_numpy(features[f"face_{index}"].copy())
                    ),
                    fusion_weights=(
                        tuple(item["fusion_weights"])
                        if item.get("fusion_weights") is not None
                        else None
                    ),
                    branch_quality=(
                        tuple(item["branch_quality"])
                        if item.get("branch_quality") is not None
                        else None
                    ),
                    branch_available=(
                        tuple(item["branch_available"])
                        if item.get("branch_available") is not None
                        else None
                    ),
                    detection=detection,
                    segmentation=None,
...
def build_local_identity_model(
    ...
    if checkpoint is not None:
        ...
        if fusion_mode is None:
            ...
            try:
                fusion_mode = normalize_fusion_mode(architecture_name)
            except ValueError:
                fusion_mode = LEGACY_CONCAT_FUSION
    else:
    ...
    joint_enabled = (
        ...
        if checkpoint is not None
            ...
            or fusion_mode in SHARED_FUSION_MODES
        )
    )
    identity_model = LocalEndToEndPetIDModel(
        nose_encoder,
