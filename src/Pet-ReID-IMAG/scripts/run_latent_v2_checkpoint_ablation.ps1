...
if ([string]::IsNullOrWhiteSpace($CheckpointDirectory)) {
    $CheckpointDirectory = Join-Path $script:PetReIDWorkspaceRoot "artifacts\runs\legacy\modern_latent_v2_s101_224_d192"
}
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    $OutputDirectory = Join-Path $script:PetReIDWorkspaceRoot ("artifacts\evaluations\latent-v2-checkpoint-ablation\{0}" -f (Get-Date -Format 'yyyyMMdd-HHmmss'))
}
