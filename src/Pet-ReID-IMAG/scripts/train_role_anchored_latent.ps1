...
if ($Microfit) {
    $configPath = "configs/modern_latent_role_anchored_microfit.yaml"
    $legacyName = "modern_latent_role_anchored_microfit_d192"
    $workstream = "role-anchored-latent-microfit"
    $purpose = "microfit"
} elseif ($Smoke) {
    $configPath = "configs/modern_latent_role_anchored_smoke.yaml"
    $legacyName = "modern_latent_role_anchored_smoke_d192"
    $workstream = "role-anchored-latent-smoke"
    $purpose = "smoke"
} else {
    $configPath = "configs/modern_latent_role_anchored_s101_224.yaml"
    $legacyName = "modern_latent_role_anchored_s101_224_d192"
    $workstream = "role-anchored-latent"
    $purpose = "train"
}
...
try {
    if (-not $SkipPreflight) {
        ...
        if ($preflightExitCode -ne 0) {
            throw "Latent Role-anchored preflight failed with code $preflightExitCode. See $preflightReport"
        }
    }
