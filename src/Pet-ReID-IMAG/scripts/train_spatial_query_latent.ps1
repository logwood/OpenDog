...
if ($Microfit) {
    $configPath = "configs/modern_latent_spatial_query_microfit.yaml"
    $legacyName = "modern_latent_spatial_query_microfit_d192"
    $workstream = "spatial-query-latent-microfit"
    $purpose = "microfit"
} elseif ($Smoke) {
    $configPath = "configs/modern_latent_spatial_query_smoke.yaml"
    $legacyName = "modern_latent_spatial_query_smoke_d192"
    $workstream = "spatial-query-latent-smoke"
    $purpose = "smoke"
} else {
    $configPath = "configs/modern_latent_spatial_query_s101_224.yaml"
    $legacyName = "modern_latent_spatial_query_s101_224_d192"
    $workstream = "spatial-query-latent"
    $purpose = "train"
}
...
try {
    if (-not $SkipPreflight) {
        $preflightArguments = @(
            "tools/preflight_role_anchored_latent.py",
        ...
        if ($preflightExitCode -ne 0) {
            throw "Spatial-query latent preflight failed with code $preflightExitCode. See $preflightReport"
        }
    }

    if ($PreflightOnly) {
        Write-Host "Spatial-query preflight passed. No training was started."
        return
    }
    ...
    if ($trainingExitCode -ne 0) {
        throw "Spatial-query training exited with code $trainingExitCode. See $consoleLog"
    }

    if ($Microfit) {
        $microfitReport = Join-Path $reportDirectory "microfit_analysis.json"
        $analysisArguments = @(
            "tools/analyze_spatial_query_microfit.py",
            "--metrics", $runMetrics,
        ...
        if ($analysisExitCode -ne 0) {
            throw "Spatial-query latent microfit quality gate failed. See $microfitReport"
        }
    }
} finally {
