...
class WorkspacePathsTest(unittest.TestCase):
    ...
    def test_windows_checkpoint_paths_relocate_without_checkout_name(self):
        cases = {
            "R:/archived-checkout/artifacts/runs/example/model.pth": (
                WORKSPACE_ROOT / "artifacts/runs/example/model.pth"
            ),
            "R:/archived-checkout/models/selected/example/model.pth": (
                SELECTED_MODELS_ROOT / "example/model.pth"
            ),
            "R:/archived-checkout/models/pretrained/dog.pt": (
                PRETRAINED_MODELS_ROOT / "dog.pt"
            ),
            "R:/archived-checkout/upstream/Pet-ReID-IMAG/configs/model.yaml": (
                SOURCE_ROOT / "configs/model.yaml"
            ),
        }
        for value, expected in cases.items():
            with self.subTest(value=value):
                self.assertEqual(resolve_legacy_path(value), expected.resolve())
