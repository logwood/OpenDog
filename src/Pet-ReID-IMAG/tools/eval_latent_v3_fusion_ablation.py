"""Evaluate V3 main/query branches and descriptor fusion without repeated inference."""
...
class _V3BranchExtractor(nn.Module):
    """Return both descriptors from one V3 backbone/workspace forward."""
...
def main():
    ...
    if model.__class__.__name__ != "LatentWorkspaceV3Baseline":
        raise TypeError(f"Expected V3 model, found {model.__class__.__name__}")
    if model.identity_query_head is None:
        raise ValueError("V3 identity query head is disabled")
    extractor = _V3BranchExtractor(model)
    ...
    print(f"Saved exact V3 fusion ablation report to {output_path}")
