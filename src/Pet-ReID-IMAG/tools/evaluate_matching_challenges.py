...
from pet_id.release_compatibility import historical_artifact_path  # noqa: E402
...
def main() -> None:
    rollback_package = historical_artifact_path(
        ROOT.parents[1], "joint-rollback-package"
    )
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--manifest",
        type=Path,
        default=historical_artifact_path(
            ROOT.parents[1], "legacy-local-gallery-manifest"
        ),
    )
    parser.add_argument(
        "--gallery-model",
        type=Path,
        default=historical_artifact_path(
            ROOT.parents[1], "joint-rollback-gallery"
        )
        / "gallery_model.json",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=EVALUATIONS_ROOT / "pet_matching_challenge"
    )
    parser.add_argument(
        "--storage-dir",
        type=Path,
        default=GALLERY_STORE_ROOT / "pet_api_gallery_challenge",
    )
