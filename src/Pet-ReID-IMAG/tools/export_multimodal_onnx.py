...
from pet_id.workspace_paths import LEGACY_RUNS_ROOT, SELECTED_MODELS_ROOT, normalize_runtime_config
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        nargs="?",
        default=SELECTED_MODELS_ROOT / "dogfacenet_joint800_v1" / "model_final.pth",
    )
    parser.add_argument(
        "--config-file",
        type=Path,
        default=SELECTED_MODELS_ROOT / "dogfacenet_joint800_v1" / "config.yaml",
    )
    parser.add_argument(
        "--manifest",
        type=Path,
        default=LEGACY_RUNS_ROOT / "dogfacenet_joint800_protocol_v1" / "blind_test_manifest.json",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=SELECTED_MODELS_ROOT / "dogfacenet_joint800_v1" / "onnx",
    )
