...
DEFAULT_OUTPUT = WORKSPACE / "artifacts/runs/agent/learned_controller"
...
def build_controller_protocol(args: argparse.Namespace) -> dict[str, Any]:
    ...
    model_train_paths = historical_model_training_manifests(WORKSPACE)
    ...
    protocol = {
        "schema_version": 1,
        "name": "learned_evidence_controller_identity_disjoint",
        "created_at": utc_now(),
        "seed": args.seed,
        "policy": {
...
def main() -> None:
    ...
    if args.images_per_identity != 4:
        raise ValueError(
            "Locked episode construction requires exactly four images per identity"
        )
