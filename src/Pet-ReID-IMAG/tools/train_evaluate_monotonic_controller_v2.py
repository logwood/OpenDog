...
from pet_id.evidence_controller_v2 import (  # noqa: E402
    DEFAULT_COSTS_V2,
    OUTPUT_NAMES_V2,
...
FORMAL = load_file_module(
    "pet_reid_formal_for_controller_v2",
    ROOT / "tools/evaluate_agent_formal_protocol.py",
)
V1 = load_file_module(
    "pet_reid_learned_controller_v1_tool",
...
DEFAULT_OUTPUT = WORKSPACE / "artifacts/runs/agent_v2/monotonic_controller_v2"
DEFAULT_V1_PROTOCOL = (
    WORKSPACE / "artifacts/runs/agent_v2/learned_controller_v1/protocol.json"
)
...
def build_protocol(args: argparse.Namespace) -> dict[str, Any]:
    v1 = read_json(args.v1_protocol)
    v1_payload = dict(v1)
    v1_hash = v1_payload.pop("protocol_sha256")
    if sha256_json(v1_payload) != v1_hash:
        raise RuntimeError("V1 protocol hash mismatch")
    development = [
        dict(row)
        for name in ("controller_train", "controller_validation")
        for row in v1["splits"][name]
    ]
    ...
    eligible.sort(
        key=lambda identity: stable_order(args.seed, "v2-fresh-test", identity)
    )
    ...
    model_train_ids, model_evidence = collect_manifest_identities(
        [
            WORKSPACE
            / "artifacts/runs/legacy/dogfacenet_joint100_protocol_v1/train_manifest.json",
            WORKSPACE
    ...
    protocol = {
        "schema_version": 1,
        "name": "monotonic_controller_fresh_three_image_test_v2",
        "created_at": utc_now(),
        "seed": args.seed,
        "policy": {
            "controller_fit": "80 V1 development identities; no V1 test identity",
            "controller_calibration": "16 identity-disjoint V1 development identities",
        ...
        "source": {
            "v1_protocol": {
                "path": str(args.v1_protocol.resolve()),
                "sha256": sha256_file(args.v1_protocol),
                "protocol_sha256": v1_hash,
            },
...
V1_OUTPUT_INDEX = {
    name: index for index, name in enumerate(V1.OUTPUT_NAMES)
}


def convert_v1_examples(
    rows: Sequence[Any],
) -> list[ScalarExample]:
    output = []
    for row in rows:
        targets = np.asarray(
            [row.targets[V1_OUTPUT_INDEX[name]] for name in OUTPUT_NAMES_V2],
            dtype=np.float32,
        )
        output.append(
            ScalarExample(
                ...
                targets=targets,
                target_mask=np.ones(len(OUTPUT_NAMES_V2), dtype=np.float32),
...
def development_examples(
    ...
    *,
    ...
    return convert_v1_examples(
        V1.make_development_examples(
            split=split,
            records=records,
            features=features,
            folds=folds,
...
def locked_test_examples(
    ...
    *,
    ...
    known_grouped = V1.records_by_identity(
        protocol["splits"]["locked_test_known"]
    )
    known_ids = sorted(known_grouped)
    gallery = V1.build_gallery(known_ids, known_grouped, features)
    queries = [
    ...
    for query, identity, known in queries:
        ...
        target_mask = np.ones(len(OUTPUT_NAMES_V2), dtype=np.float32)
        target_mask[OUTPUT_NAMES_V2.index("recapture_gain")] = 0.0
        target_mask[OUTPUT_NAMES_V2.index("temporal_consistency")] = 0.0
        targets = np.asarray(
            [target_map[name] for name in OUTPUT_NAMES_V2], dtype=np.float32
        )
        facts = {
            "known": known,
...
def fit_model(
    ...
    *,
    ...
    if best_state is None:
        raise RuntimeError("No V2 checkpoint was selected")
    ...
    return model, normalizer, {
        ...
        "positive_counts": {
            ...
            for index, name in enumerate(OUTPUT_NAMES_V2)
        },
        "positive_weights": {
...
def calibrate_temperatures(
    ...
    output = np.ones(len(OUTPUT_NAMES_V2), dtype=np.float32)
    candidates = np.geomspace(0.25, 4.0, 121)
    for index in range(len(OUTPUT_NAMES_V2)):
...
def select_weight_decay(
    ...
    grouped = V1.records_by_identity(records)
    identities = sorted(
        grouped,
        key=lambda identity: stable_order(args.seed, "v2-cv", identity),
    )
    identity_folds = [
        set(
            identity
...
def evaluate_policy(
    ...
    probability_map = {
        row.example_id: {
            ...
            for head, name in enumerate(OUTPUT_NAMES_V2)
        }
    ...
    for episode_id, stages in sorted(group_stages(rows).items()):
        ...
        if action == "defer_review":
            ...
            raise RuntimeError(f"Unexpected V2 final action: {action}")
        decisions.append(
            {
                "episode_id": episode_id,
...
def export_onnx(
    ...
    return {
        ...
        "sha256": sha256_file(path),
        "output_names": list(OUTPUT_NAMES_V2),
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--config-file", type=Path, default=FORMAL.DEFAULT_BIFOR_PACKAGE / "config.yaml")
    parser.add_argument("--identity-weights", type=Path, default=FORMAL.DEFAULT_SEMANTIC_PACKAGE / "model_final.pth")
    parser.add_argument("--onnx-model", type=Path, default=FORMAL.DEFAULT_BIFOR_PACKAGE / "onnx/pet_embedding.onnx")
    parser.add_argument(
        "--body-detector",
        type=Path,
    ...
    parser.add_argument("--consult-cost", type=float, default=DEFAULT_COSTS_V2["consult_expert"])
    parser.add_argument("--reject-cost", type=float, default=DEFAULT_COSTS_V2["reject_unknown"])
    parser.add_argument("--review-cost", type=float, default=DEFAULT_COSTS_V2["defer_review"])
...
def main() -> None:
    ...
    args.v1_protocol = args.v1_protocol.expanduser().resolve()
    ...
    checkpoint = args.output_dir / "controller_v2.pt"
    torch.save(
        {
            "schema_version": 2,
    ...
    if not args.skip_onnx_export:
        onnx = export_onnx(
            ...
            normalizer,
            temperatures,
            args.output_dir / "controller_v2.onnx",
        )
