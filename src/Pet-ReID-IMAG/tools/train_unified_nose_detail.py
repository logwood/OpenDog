#!/usr/bin/env python3
"""Tune the author-reproduced nose encoder in the spatial-detail graph.

Only the nose adapter and bounded parent/detail refiners are trainable. Geometry,
ArcFace, and the complete nose backbone remain frozen.
"""
...
def verify_protocol_sources(
    ...
    if lock.get("status") != "LOCKED_UNSCORED":
        raise RuntimeError("Stage 1 requires a LOCKED_UNSCORED protocol")
    ...
    source_paths: dict[str, set[Path]] = {}
    ...
    for split, manifest_path in manifests.items():
        ...
        identities[split] = {str(row["identity"]).casefold() for row in records}
        source_paths[split] = set()
        for row in records:
            source = workspace_path(row["source_path"])
            if source in source_paths[split]:
                raise RuntimeError(f"Duplicate {split} source path: {source}")
    ...
    overlap_identities = identities["train"] & identities["validation"]
    overlap_images = source_paths["train"] & source_paths["validation"]
    if overlap_identities or overlap_images:
        raise RuntimeError(
            "Locked train/validation isolation failed: "
            f"{len(overlap_identities)} identities, {len(overlap_images)} images"
        )
    return {
        "splits": split_rows,
        "train_identities": len(identities["train"]),
        "validation_identities": len(identities["validation"]),
        "identity_overlap": len(overlap_identities),
        "image_overlap": len(overlap_images),
        "verified_images": sum(split_rows.values()),
...
STAGE1_TRAINABLE_PREFIXES = (
    "parent_model.base_model.nose_adapter.",
    "parent_model.base_model.fusion.",
    "parent_model.refiner.",
    "refiner.",
)

STAGE1_FORBIDDEN_FRAGMENTS = (
    "geometry_frontend",
    "identity_encoder",
    "nose_encoder.model",
)
...
def configure_parameter_scope(model: nn.Module) -> list[tuple[str, nn.Parameter]]:
    model.requires_grad_(False)
    trainable: list[tuple[str, nn.Parameter]] = []
    for name, parameter in model.named_parameters():
        if name.startswith(STAGE1_TRAINABLE_PREFIXES):
            if any(fragment in name for fragment in STAGE1_FORBIDDEN_FRAGMENTS):
                raise RuntimeError(f"Forbidden stage-1 parameter matched the whitelist: {name}")
            parameter.requires_grad_(True)
            trainable.append((name, parameter))
    if not trainable:
        raise RuntimeError("The stage-1 trainable whitelist matched no model parameters")
    actual = {name for name, parameter in model.named_parameters() if parameter.requires_grad}
    expected = {name for name, _ in trainable}
    if actual != expected:
        raise RuntimeError(f"Stage-1 trainable scope mismatch: {sorted(actual ^ expected)}")
    return trainable


def parameter_audit(model: nn.Module, trainable: list[tuple[str, nn.Parameter]]) -> dict[str, Any]:
    forbidden = [
        name
        for name, parameter in model.named_parameters()
        if parameter.requires_grad
        and any(fragment in name for fragment in STAGE1_FORBIDDEN_FRAGMENTS)
    ]
    if forbidden:
        raise RuntimeError(f"Frozen encoder or geometry parameter became trainable: {forbidden}")
...
def set_frozen_stage1_modules_eval(model: nn.Module) -> None:
...
def gradient_audit(
    ...
    if forbidden_with_grad:
        raise RuntimeError(f"Frozen stage-1 parameter received a gradient: {forbidden_with_grad}")
    nonzero = [
        name
    ...
    if not nonzero:
        raise RuntimeError("No non-zero gradient reached the stage-1 adapter/refiner whitelist")
    return {
        "nonzero_trainable_gradient_tensors": len(nonzero),
        "trainable_gradient_tensors": sum(parameter.grad is not None for _, parameter in trainable),
        "forbidden_gradient_tensors": len(forbidden_with_grad),
    }
...
def main() -> None:
    ...
    source_role = config["model"].get("source_role")
    model_path = (
        historical_run_path(WORKSPACE, str(source_role))
        if source_role
        else workspace_path(config["model"]["checkpoint"])
    )
    ...
    print(json.dumps({"protocol_source_audit": source_audit}, indent=2), flush=True)
    ...
    configured_prefixes = tuple(config["model"]["trainable_scope"]["prefixes"])
    if configured_prefixes != STAGE1_TRAINABLE_PREFIXES:
        raise RuntimeError(
            "The configured trainable prefixes do not exactly match the locked stage-1 whitelist"
        )
    for key in ("freeze_geometry", "freeze_arcface", "freeze_nose_backbone"):
        if not bool(config["model"]["trainable_scope"][key]):
            raise RuntimeError(f"Stage 1 requires {key}=true")
    trainable = configure_parameter_scope(model)
    scope_audit = parameter_audit(model, trainable)
    print(json.dumps({"parameter_audit": scope_audit}, indent=2), flush=True)
        ...
        [
            {
                ...
                "lr": float(optimizer_cfg["adapter_refiner_lr"]),
                "name": "adapter_refiners",
            },
    ...
    calculated_effective_batch = (
        identities_per_batch * images_per_identity * accumulation
    )
    if calculated_effective_batch != int(training["effective_batch_size"]):
        raise RuntimeError(
            "Configured effective batch size mismatch: "
            f"{calculated_effective_batch} != {training['effective_batch_size']}"
        )
    ...
    smoke_initial_trainable = (
        {name: parameter.detach().clone() for name, parameter in trainable}
        if smoke_limit
        else {}
    )
    smoke_update_audit: dict[str, Any] | None = None
    for epoch in range(start_epoch, epochs):
        model.train()
        set_frozen_stage1_modules_eval(model)
        classifier.train()
        ...
        for microbatch, indices in enumerate(batches):
            ...
            if not torch.isfinite(loss):
                raise FloatingPointError("Non-finite spatial-detail loss")
            (loss / accumulation).backward()
            smoke_gradient_audit = None
            if smoke_limit:
                smoke_gradient_audit = gradient_audit(model, trainable)
            for key, value in {
                "loss": loss,
                "cross_entropy": ce,
                "triplet": triplet,
            ...
            should_step = (
                ...
                or (smoke_limit and seen_microbatches + 1 >= smoke_limit)
            )
            if should_step:
                ...
                if smoke_limit:
                    changed = [
                        ...
                        if not torch.equal(
                            parameter.detach(),
                            smoke_initial_trainable[name],
                        )
                    ]
                    if not changed:
                        raise RuntimeError(
                            "The smoke optimizer step changed no stage-1 parameters"
                        )
                    smoke_update_audit = {
                        "optimizer_step_executed": True,
                        "changed_trainable_tensors": len(changed),
                        "forbidden_parameters_in_optimizer": 0,
                    }
                optimizer.zero_grad(set_to_none=True)
                global_step += 1
            seen_microbatches += 1
                    ...
                    json.dumps(
                        {
                            ...
                            "stage1_gradient_audit": smoke_gradient_audit,
                            "cuda_memory_gib": (
                ...
                json.dumps(
                    {
                        ...
                        "parameter_update_audit": smoke_update_audit,
                    },
                    indent=2,
                )
            )
        ...
        if is_best and epoch + 1 >= int(config["selection"]["save_best_only_after_epoch"]):
            ...
            payload = create_highres_checkpoint(
                ...
                training={
                    ...
                    "optimizer_step": global_step,
                    "parameter_audit": scope_audit,
                    "standard_recipe": "stage 1 / 10 epochs / frozen encoders and geometry / Adam / CE + triplet + circle / identity-disjoint validation",
                    "seed": seed,
                },
