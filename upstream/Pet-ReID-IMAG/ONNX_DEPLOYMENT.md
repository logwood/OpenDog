...
`models/dogfacenet_semantic_v3_v1/onnx/pet_embedding.onnx`. It emits a
...
python tools/multimodal_inference.py D:\images `
  --config-file models/dogfacenet_semantic_v3_v1/config.yaml `
  --backend onnx `
  --onnx-model models/dogfacenet_semantic_v3_v1/onnx/pet_embedding.onnx `
  --onnx-provider cuda `
  --onnx-warmup-batches 1,4,8 `
  --output-dir logs/onnx_inference
```
...
python tools/build_pet_gallery_model.py `
  ...
  models/dogfacenet_semantic_v3_v1/model_final.pth `
  --config-file models/dogfacenet_semantic_v3_v1/config.yaml `
  --backend onnx `
  --onnx-model models/dogfacenet_semantic_v3_v1/onnx/pet_embedding.onnx `
  --onnx-provider cuda `
  --production-only `
  --output-dir models/local_pet_gallery_semantic_v3_onnx_v1
```
...
```powershell
python tools/validate_pet_gallery.py D:\new-images `
  --gallery-model models/local_pet_gallery_semantic_v3_onnx_v1/gallery_model.json `
  --backend onnx `
  --onnx-provider cuda `
  --production-only `
  --output-dir logs/new_image_run
```
...
python tools/benchmark_multimodal_runtime.py D:\images\sample.jpg `
  ...
  --warmup-runs 2 `
  --iterations 10 `
  --output logs/onnx_end_to_end_benchmark.json
```
