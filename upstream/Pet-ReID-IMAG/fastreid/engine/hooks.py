...
class PeriodicCheckpointer(_PeriodicCheckpointer, HookBase):
    ...
    def before_train(self):
        ...
        if len(self.trainer.cfg.DATASETS.TESTS) == 0:
            self.metric_name = None
        elif len(self.trainer.cfg.DATASETS.TESTS) == 1:
    ...
    def after_epoch(self):
        ...
        metric_dict = dict(
            ...
            if self.metric_name is not None and self.metric_name in storage.latest()
            else -1
        )
        if self.metric_name is None:
            # Final train-on-all-data runs have no honest validation metric.
            # Alternate between two recovery slots instead of accumulating a
            # large model file at every checkpoint epoch. If a save is
            # interrupted, the other slot remains a valid fallback.
            epoch = int(self.trainer.epoch)
            state = {"epoch": epoch, **metric_dict}
            if epoch >= self.max_epoch - 1:
                self.checkpointer.save("model_final", **state)
            elif (epoch + 1) % self.period == 0:
                save_index = ((epoch + 1) // self.period - 1) % 2
                self.checkpointer.save(f"model_recent_{save_index}", **state)
            return
...
class LRScheduler(HookBase):
    ...
    def __init__(self, optimizer, scheduler):
        ...
        self._scale = 0
        self._optimizer_step_seen = False
    ...
    def after_step(self):
        ...
        self._optimizer_step_seen |= optimizer_step_succeeded
    ...
    def after_epoch(self):
        ...
        if (
            self._optimizer_step_seen
            and next_iter > self.trainer.warmup_iters
...
class EvalHook(HookBase):
    ...
    def after_train(self):
        ...
        if (
            self._period > 0
            and next_epoch % self._period != 0
            and next_epoch >= self.trainer.max_epoch
        ):
            self._do_eval()
...
class LayerFreeze(HookBase):
    ...
    def before_step(self):
        ...
        if self.trainer.iter < self.freeze_iters:
            if not self.is_frozen:
                ...
                # Evaluation and third-party hooks may recursively call
                # model.train(). Reassert the frozen-mode invariant before
                # every optimization step without repeating the log message.
                self._set_frozen_layers_eval()

        # Recover original layers status
        if self.trainer.iter >= self.freeze_iters and self.is_frozen:
            self.open_all_layer()

    def _set_frozen_layers_eval(self):
    ...
    def freeze_specific_layer(self):
        ...
        # Change BN and other training-sensitive operations in freeze layers
        # to eval mode while leaving non-frozen top-level modules trainable.
        self._set_frozen_layers_eval()
