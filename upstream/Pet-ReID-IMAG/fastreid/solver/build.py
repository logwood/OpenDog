...
def _generate_optimizer_class_with_freeze_layer(
    ...
    counter_key = "_freeze_step_count"

    def infer_legacy_step_count(state_dict):
        """Recover progress from Adam-like legacy checkpoints when possible."""
        steps = []
        ...
        return min(max(steps, default=0), freeze_iters)

    @torch.no_grad()
    def optimizer_wfl_step(self, closure=None):
        freeze_step_count = int(getattr(self, counter_key, 0))
        if freeze_step_count < freeze_iters:
            setattr(self, counter_key, freeze_step_count + 1)
            param_ref = []
            grad_ref = []
            for group in self.param_groups:
    ...
    def optimizer_wfl_state_dict(self):
        state_dict = optimizer.state_dict(self)
        state_dict[counter_key] = int(getattr(self, counter_key, 0))
        return state_dict

    def optimizer_wfl_load_state_dict(self, state_dict):
        # Do not mutate the checkpoint object owned by Checkpointer.
        state_dict = dict(state_dict)
        freeze_step_count = state_dict.pop(counter_key, None)
        if freeze_step_count is None:
            # Released/older checkpoints predate the explicit counter. Adam's
            # normal parameter state still records the number of successful
            # optimizer steps, which gives the correct resume point here.
            freeze_step_count = infer_legacy_step_count(state_dict)
        result = optimizer.load_state_dict(self, state_dict)
        setattr(
            self,
            counter_key,
            min(max(int(freeze_step_count), 0), freeze_iters),
        )
        return result
    ...
    OptimizerWithFreezeLayer = type(
        ...
        (optimizer,),
        {
            ...
            "state_dict": optimizer_wfl_state_dict,
            "load_state_dict": optimizer_wfl_load_state_dict,
        },
    )
...
def build_lr_scheduler(cfg, optimizer, iters_per_epoch):
    # Tiny smoke/debug schedules may fit entirely inside warmup.  Modern
    # PyTorch rejects CosineAnnealingLR(T_max=0), so retain one valid epoch.
    max_epoch = max(1, cfg.SOLVER.MAX_EPOCH - max(
