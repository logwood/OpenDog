...
实际前端工程位于 `frontend/pet-reid-web`，使用 React、TypeScript 和 Sites/Vinext。
...
当前准备状态：前端构建、TypeScript 与 lint 检查通过；Java 10/10 测试通过；允许 Origin
的真实预检返回 200，未授权 Origin 返回 403。验证记录位于
`logs/frontend_cors_smoke/result.json`。
