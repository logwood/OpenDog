...
export const API_BASE_URL = (
  process.env.NEXT_PUBLIC_PET_REID_API_BASE_URL ?? "http://127.0.0.1:8080"
).replace(/\/$/, "");
...
export function errorMessage(error: unknown): string {
  ...
  if (error instanceof TypeError) {
    return "无法连接本地 Java 网关，请确认 8080 端口已启动并允许当前前端地址。";
  }
...
export const petReIdApi = {
  ...
  historyImageUrl: (historyId: string) =>
    API_BASE_URL + "/v1/history/" + encodeURIComponent(historyId) + "/image",
...
export function fusionState(descriptor: DescriptorMetadata | null): "joint" | "fallback" | "unknown" {
