...
- JDK 21
- Maven 3.9+
- 已按仓库根目录 `PET_API.md` 配好的 Python ONNX Runtime CUDA 环境
...
D:\CondaData\envs\torch312\python.exe tools\serve_pet_api.py `
  ...
  --identity-weights models\dogfacenet_semantic_v3_v1\model_final.pth `
...
```powershell
# 10 个测试：HTTP 协议、multipart、查询参数、错误映射、CORS、控制器与完整上下文
...
本机已完成一次真实链路验证：10/10 Java 测试通过；Java multipart API 录入 2 只宠物、
...
logs/java_gateway_semantic_v3_e2e/e2e_result.json。
