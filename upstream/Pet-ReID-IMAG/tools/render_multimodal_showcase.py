...
def render_overview(repo: Path, output: Path) -> None:
    protocol = load_json(repo / "logs/dogfacenet_protocol32/protocol_completion.json")
    validation = load_json(repo / "logs/dogfacenet_protocol32/validation_selection.json")
    blind = load_json(repo / "logs/dogfacenet_protocol32/blind_test_result.json")
    expanded = load_json(repo / "logs/dogfacenet_expanded20/comparison.json")
    expanded_manifest = load_json(repo / "logs/dogfacenet_expanded20/manifest.json")
...
def render_query_board(repo: Path, output: Path) -> None:
    comparison = load_json(repo / "logs/dogfacenet_expanded20/comparison.json")
    manifest = load_json(repo / "logs/dogfacenet_expanded20/manifest.json")
...
def render_evidence(repo: Path, output: Path) -> None:
    blind = load_json(repo / "logs/dogfacenet_protocol32/blind_test_result.json")
    blind_manifest = load_json(repo / "logs/dogfacenet_protocol32/blind_test_manifest.json")
    expanded = load_json(repo / "logs/dogfacenet_expanded20/comparison.json")
    expanded_manifest = load_json(repo / "logs/dogfacenet_expanded20/manifest.json")
...
def main() -> None:
    ...
    render_overview(repo, output / "pet-reid-showcase-overview.png")
    render_query_board(repo, output / "pet-reid-expanded20-query-board.png")
    render_evidence(repo, output / "pet-reid-results-evidence.png")
    summary = {
    ...
    (repo / "logs/dogfacenet_expanded20/showcase_outputs.json").write_text(
