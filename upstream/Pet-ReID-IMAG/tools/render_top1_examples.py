...
def render_examples(repo: Path, output: Path) -> None:
...
def render_wanda_before_after(repo: Path, output: Path) -> None:
    comparison = load_json(repo / "logs/dogfacenet_expanded20/comparison.json")
    manifest = load_json(repo / "logs/dogfacenet_expanded20/manifest.json")
    records = manifest["records"]
...
def main() -> None:
    ...
    render_examples(repo, output_dir / "pet-reid-top1-examples.png")
    render_wanda_before_after(repo, output_dir / "pet-reid-top1-wanda-before-after.png")
