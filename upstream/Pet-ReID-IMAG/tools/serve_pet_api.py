...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--storage-dir",
        type=Path,
        default=Path("models/pet_api_gallery_semantic_v3_v1"),
    )
    parser.add_argument(
        "--seed-gallery-model",
        type=Path,
...
def main() -> None:
    ...
    identity_weights = args.identity_weights.expanduser().resolve()
    ...
    store = PetGalleryStore(args.storage_dir)
    ...
    if args.seed_gallery_model is not None:
        imported = service.import_gallery_model(args.seed_gallery_model)
