...
def load_queries(args) -> list[dict]:
    ...
    if args.manifest:
        ...
        rows.extend(
            {
                "path": Path(record["library_path"]).resolve(),
...
def render_results(
    ...
    for index, query in enumerate(queries):
        ...
        reference_image = thumbnail(Path(references[top_ref_index]["path"]), (250, 230))
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("logs/local_pet_gallery_validation"))
    ...
    onnx_model = (
        ...
        if args.onnx_model is not None
            ...
            recorded_onnx_model
            or "models/dogfacenet_joint800_v1/onnx/pet_embedding.onnx"
        ).resolve()
    )
    ...
    config = Path(model["config_file"])
    checkpoint = Path(model["selected_checkpoint"])
